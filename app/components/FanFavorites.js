...
export default function FanFavorites() {
  ...
  const totalPages = Math.ceil(favorites.length / 6);
  ...
  function goNext() {
    ...
    setTimeout(() => { setPage(p => p + 1); setTransitioning(false); }, 300);
  }
  ...
  return (
    <section style={{ padding: '0 80px 80px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '32px' }}>
        <span style={{ fontSize: '11px', letterSpacing: '4px', textTransform: 'uppercase', color: '#8a7f6a', fontWeight: '500', whiteSpace: 'nowrap', fontFamily: 'sans-serif' }}>Fan Favorites</span>
        <div style={{ flex: 1, height: '0.5px', background: '#1a1610' }} />
        <span style={{ fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', color: '#8a7f6a', fontWeight: '400', whiteSpace: 'nowrap' }}>Ordered by user rating</span>
      </div>
      ...
      >
        ...
        <div style={{
          ...
          zIndex: 10, opacity: showLeftArrow && page > 0 ? 1 : 0,
          transition: 'opacity 0.2s ease', pointerEvents: showLeftArrow && page > 0 ? 'all' : 'none',
        }}>
          <button onClick={goPrev} style={{
            background: 'rgba(10,10,10,0.9)', border: '0.5px solid #3a3020', color: '#f0e8d0',
            width: '40px', height: '40px', borderRadius: '50%', cursor: 'pointer', fontSize: '20px',
            display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'border-color 0.2s, box-shadow 0.2s',
          }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = '#c9a84c'; e.currentTarget.style.boxShadow = '0 0 12px rgba(201,168,76,0.5)'; }}
        ...
        <div style={{
          ...
          zIndex: 10, opacity: showRightArrow && page < totalPages - 1 ? 1 : 0,
          transition: 'opacity 0.2s ease', pointerEvents: showRightArrow && page < totalPages - 1 ? 'all' : 'none',
        }}>
        ...
        <div style={{
          ...
          transform: transitioning ? `translateX(${slideDir === 'right' ? '-20px' : '20px'})` : 'translateX(0)',
          ...
          {visible.map(film => (
            ...
            >
              <div style={{
                ...
                boxShadow: hoveredId === film.id ? '0 12px 32px rgba(0,0,0,0.9), 0 0 0 0.5px rgba(201,168,76,0.3)' : '0 4px 12px rgba(0,0,0,0.5)',
              }}>
                {film.poster_file ? (
                ...
                <div style={{ position: 'absolute', top: '8px', right: '8px', fontSize: '10px', color: '#c9a84c', letterSpacing: '1px', fontWeight: '600' }}>
                  ★ {film.all_time_rating}
                </div>
              ...
              <div style={{ fontSize: '10px', color: '#8a7f6a', letterSpacing: '1px', textTransform: 'uppercase', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {film.director_name} · {film.year}
              </div>
            </div>
          ))}
        </div>
        ...
        {totalPages > 1 && (
          <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginTop: '32px' }}>
            {Array.from({ length: totalPages }).map((_, i) => (
              <div key={i} onClick={() => i < page ? goPrev() : goNext()} style={{
                width: i === page ? '20px' : '6px', height: '3px',
                background: i === page ? '#c9a84c' : '#3a3020',
                borderRadius: '2px', transition: 'all 0.3s ease', cursor: 'pointer',
              }} />
            ))}
          </div>
        )}
